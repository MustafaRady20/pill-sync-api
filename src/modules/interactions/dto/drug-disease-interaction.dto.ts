...
export class CreateDrugDiseaseInteractionDto {
  ...
  @ApiProperty({ example: 'NSAIDs are contraindicated in active peptic ulcer disease', required: false })
  @IsOptional()
  @IsString()
  description?: string;

  @ApiProperty({ example: 'Inhibition of COX-1 reduces mucosal protection', required: false })
  @IsOptional()
  @IsString()
  mechanism?: string;

  @ApiProperty({ example: '500mg twice daily with food', required: false, description: 'For Indication relation only' })
  @IsOptional()
  @IsString()
...
export class UpdateDrugDiseaseInteractionDto extends PartialType(CreateDrugDiseaseInteractionDto) {}