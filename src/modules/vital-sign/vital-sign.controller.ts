...
export class VitalSignsController {
  ...
  @ApiQuery({ name: "type", required: false, description: "Filter by vital sign type (e.g. blood_pressure)" })
  @ApiResponse({ status: 200, description: "List of vital signs for the patient." })
  findByPatient(
    @Param("patientId") patientId: string,
    @Query("type") type?: string,
  ) {
    if (type) return this.vitalSignsService.findByPatientAndType(patientId, type);
